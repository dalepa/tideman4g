...
float SENSORHEIGHTINFEET=92/12;  // The hight of the botton of the sensor in feet
int mHourCnt = 0;
...
void setup() {
  ...
  loggingIP = Cellular.resolve("bi.pancamo.com");

  Particle.publish("Tideman3", "IP="+loggingIP.toString());
  ...
  Serial.println("Tideman3 Starting " + myID + " " +loggingIP.toString());
  ...
  pinMode(led, OUTPUT);
    ...
    Particle.publish("BME Status", "bme.begin() ID=" + myID);
  ...
  //SONAR
  mySerial.begin(9600);
  Serial.println("Sonar Begin 9600 OK");
}
...
void getFUEL()
{
  ...
  toInflux("BSoC", String(fuel.getSoC()));
  toInflux("BNSoC", String(fuel.getNormalizedSoC()));


  toInflux("BPowSrc", String(System.powerSource()));
  toInflux("BUp", String(System.uptime()));
  toInflux("BBatC", String(System.batteryCharge()));
  toInflux("BBatSS", String(System.batteryState()));
...
void getCELL()
{
  ...
  sig = Cellular.RSSI();
  toInflux("BSS", String(sig.getStrength()));
  toInflux("BSQ", String(sig.getQuality()));
...
void getBME680()
{
    ...
    Particle.publish("BME", "Failed to perform reading of BME :(");
    Serial.print("BME Failed to perform reading on BME: ");
  } else {
    ...
    toInflux("tmp", String((temperatureInC * 9/5) + 32));
    toInflux("hum", String(relativeHumidity));
    toInflux("psi", String(pressureHpa));
    toInflux("gas", String(gasResistanceKOhms));
    toInflux("alt", String(approxAltitudeInM));
...
void getSonarDistance()
{
  ...
  if (count == 400) {
    ...
    toInflux("WAvg", String(WaterLevelAvg));
    toInflux("WHigh", String(WaterLevelHigh));
    toInflux("WLow", String(WaterLevelLow));
    toInflux("WDist", String(avg/12));
    ...
    toInflux("BLoop",String(LoopCycleDuration )); 
    

    // Calculate Total Bytes Sent
    toInflux("BSent", String((totalBufferLen/LoopCycleDuration) * 60.0 * 60.0 * 24.0 * 30.0));
    toInflux("BLen", String(totalBufferLen));
